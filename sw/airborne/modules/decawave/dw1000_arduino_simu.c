...
static void dw1000_send_pos()
{
  //Position of the Aircraft
  struct EnuCoor_f *pos = stateGetPositionEnu_f();
  //Position of the Decawave system
  float da_x = WaypointX(WP_BALISE);
  float da_y = WaypointY(WP_BALISE);
  float x = ((pos->x - da_x) * cos(landing_direction) + (pos->y - da_y) * sin(landing_direction)) ;
  float y = ((pos->y - da_y) * cos(landing_direction) -(pos->x - da_x) * sin(landing_direction)) ;
  uint32_t now_ts = get_sys_time_usec();
  //Send POSITION_ESTIMATE type message
  AbiSendMsgPOSITION_ESTIMATE(GPS_DW1000_ID, now_ts, x, y, pos->z,NOISE_X,NOISE_Y,NOISE_Z);
}
